 ...
 * automatically when available, then prompts the system installer.
 *
...
fun AutoUpdateHost(content: @Composable () -> Unit) {
    ...
    // Kick off automatic check once per process
    LaunchedEffect(Unit) {
    ...
    Box(Modifier.fillMaxSize()) {
        ...
        when (val p = phase) {
            AutoUpdatePhase.Idle -> Unit
            AutoUpdatePhase.Checking -> {
                // Non-blocking subtle indicator — skip UI noise for brief check
            }
            AutoUpdatePhase.Downloading -> {
                AlertDialog(
                    onDismissRequest = { /* block dismiss while downloading */ },
                    title = { Text("Updating QuickGit") },
                    text = {
                        Column {
                            Text(
                                "Downloading ${latestName ?: "latest"} from " +
                                    "${AppUpdateConfig.OWNER}/${AppUpdateConfig.REPO}…"
                            )
